...
export const GET = withApiAuthRequired(async function getAllReviews(
  req: NextRequest
) {
  ...
  try {
    ...
    let reviews = await dbRtns.findAll(
      db,
      recipeReviewCollection,
      { sub: user.sub },
      {}
    ...
    try {
      reviews.forEach((review) => {
        ...
        if (isNaN(rating) || rating < 1 || rating > 5) {
          throw new Error(
            "Rating must be a number or be value between 1 and 5"
          );
        }
        delete review.sub;
    ...
    result.message =
      updatedReviews.length > 0
        ? `${updatedReviews.length} reviews found.`
        : "No reviews found.";
...
export const POST = withApiAuthRequired(async function postRecipeReview(
  req: NextRequest
) {
  //rate limiting
  if (!rateLimit(req, 100, 15 * 60 * 1000)) {
  ...
  if (req.body) {
    ...
    try {
      ...
      if (!isValidAvatarUrl(user.picture)) {
        return NextResponse.json(
          { error: "Invalid user picture URL." },
          { status: 404 }
        );
      }
      (body.newReview.userNickname = user.email_verified
        ? user.name
        : user.nickname),
        (body.newReview.userPictureUrl = user.picture);

      if (
        isNaN(body.newReview.rating) ||
        body.newReview.rating < 1 ||
        body.newReview.rating > 5
      ) {
        return NextResponse.json(
      ...
      // Also update the original recipe in the shared recipes collection
      // Find the recipe that this review belongs to
      const recipeId = body.newReview.recipeId;
      if (recipeId) {
        // Try to find the recipe in shared recipes collection
        let recipe = await dbRtns.findOne(db, sharedRecipeCollection, {
          _id: new ObjectId(recipeId),
        });

        // If not found with ObjectId, try with string
        if (!recipe) {
          recipe = await dbRtns.findOne(db, sharedRecipeCollection, {
            _id: recipeId,
          });
        }
        ...
        if (recipe) {
          ...
          const allReviews = await dbRtns.findAll(
            db,
            recipeReviewCollection,
            { recipeId: recipeId },
            {}
          ...
          // Update the recipe with the new reviews array
...
export const PUT = withApiAuthRequired(async function putRecipeOnSocial(
  ...
  if (req.body) {
    ...
    try {
      ...
      if (
        isNaN(body.review.rating) ||
        body.review.rating < 1 ||
      ...
      await dbRtns.updateOne(
        db,
        recipeReviewCollection,
        { _id: new ObjectId(body.recipe._id) },
        body.recipe
      );
...
export const DELETE = withApiAuthRequired(async function deleteReview(
  req: NextRequest
) {
  //rate limiting
  if (!rateLimit(req, 100, 15 * 60 * 1000)) {
  ...
  try {
    let db = await dbRtns.getDBInstance();
    let response = await dbRtns.findOne(db, recipeReviewCollection, {
      _id: new ObjectId(reviewId),
    });

    if (!response) {
      return NextResponse.json(
        { error: "Selected review does not exist." },
        { status: 404 }
      );
    }
    ...
    if (user.sub !== response.userId) {
      return NextResponse.json(
        { error: "The user is not authorized to delete this review." },
        { status: 401 }
      );
    }

    //response is the object deleted
    response = await dbRtns.deleteOne(db, recipeReviewCollection, {
      _id: reviewId,
    });
    if (response) {
