...
export async function GET(req: NextRequest, res: NextResponse) {
  ...
  if (!rateLimit(req, 500, 15 * 60 * 1000)) {
    ...
    if (criteria)
      switch (criteria) {
        ...
        case API_DRINK_ROUTES.ingredients: {
          let db = await dbRtns.getDBInstance();
          let allIngredients = await dbRtns.findAll(
            db,
            ingredientCollection,
            {},
            {}
        ...
        case API_DRINK_ROUTES.filteredDrinks: {
          ...
          let db = await dbRtns.getDBInstance();
          let filteredDrinks = await dbRtns.findAll(
            db,
            recipeCollection,
            criteria,
            {}
        ...
        default: {
          return NextResponse.json(
            { error: "Invalid search criteria passed in" },
            { status: 400 }
          );
        }
      }
      ...
      return NextResponse.json(
        { error: " Criteria was not passed in" },
        { status: 400 }
      );
  } catch (err) {
